...
-   The two 2024 MVPs, Aaron Judge and Shohei Ohtani, are the two cases with the most home runs (though not the highest average swing speed).
...
-   There is a slight positive correlation between average swing speed and strikeout % but the points are more spread out than in the previous scatterplot. Overall, there does not seem to be a significant trade-off for increasing bat speed.
...
-   Apart from the one-to-one correlations, the strongest relationships between attributes are between strikeout % and whiff % and also between average swing speed and hard hit %.

-   The areas of little to no correlation are between strikeout % and intentional walks, average launch angle and average swing speed, and hard hit % and average launch angle. This is important considering average launch angle is a metric commonly associated with home runs.
...
@homeRunsLeague looks at the total number of home runs hit by players in the American and National Leagues.
...
# Bar Graph: Average Home Runs by League
...
@homeRunsVsWalksLeaguePos displays a visualization divided into the three league designations with points plotted by walks and homeruns. The points are color coded by the primary position of the MLB player.
...
-   The positions "Infielder" and "Outfielder" are mostly evenly dispersed. On the whole, there are more infielders than outfielders by virtue of the layout of a baseball field.

-   While there are not enough DHs to be significant, it is interesting to note that the National League's potential upper outliers are all DHs, and the American League's potential upper outliers are outfielders.

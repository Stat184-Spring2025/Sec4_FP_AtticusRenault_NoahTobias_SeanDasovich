...
-   The data sets were joined by name.
-   The merged table combines both the heavily quantitative variables at the heart of the first research question with the categorical variable of birthplace.

### Filtering and Cleaning

-   The bulk of filtering had been done prior to reading the csv into the environment.
...
-   Description: The final dataset has a combination of the statistics relevant to determining correlation between bat speed and other quantitative variables as well as the variable of birthplace. Additionally, the data set contains the first and last names of the 129 qualified players.
-   Purpose: This dataset streamlines the analysis of bat speed by only including statistics that are likely to be influenced by bat speed (with the exception of birthplace, which is more unconventional). 
-   Case: Each row represents an MLB player, with columns capturing their batting statistics and birthplace.
...
-   The primary dataset was constructed using data on Baseball Savant -- a database accessible from any search engine.
...
The data does not include sensitive information, yet we recognize the ethical considerations that arise when working with location data (player birthplaces). The relations made between location and other attributes are performed in a way to not disparage a player's background. Instead, these connections serve to prove the utility of global perspectives in data analysis. All data can be found publicly, and we strove to remove interpretation from biased reasoning.

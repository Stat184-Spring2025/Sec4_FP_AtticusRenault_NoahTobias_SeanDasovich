...
-   Description: Baseball Reference is part of an independent encyclopedia of sports statistics [@baseballreference]. It provides a lot of the same information as Baseball Savant but is better for finding individual player information. Baseball Reference also has tables of aggregate season data from which we could find batters' league (American or National) and position (infield, outfield, neither, both). The original table for 2024 has 34 columns and 741 rows.
...
@homeRunvsAvgSwingSpeed visualizes the correlation between average bat speed (interchangeable with average swing speed) and home runs hit by qualified MLB batters. Home runs are increasingly emphasized in modern baseball, so this scatterplot could shed some light on how these metrics are related.

```{r}
#| label: fig1-homeRunvsAvgSwingSpeed
...
@kvsAvgSwingSpeed is another scatterplot, this time comparing average swing speed with a batter's strikeout % (the percent of plate appearances the result in a strikeout). With more emphasis on bigger hits, this visualization can determine if there is a trade-off required.

```{r}
#| label: fig2-kvsAvgSwingSpeed
...
@correlationHeatmap shows the relative strength of how each batting statistics influences every other statistic. The diagonal will be perfect correlation, and the matrix as a whole is symmetric since metrics are mirrored on either axis.

```{r}
#| label: fig3-correlationHeatmap
...
@avgSwingSpeedLeague breaks down average swing speed by league. The "2LG" category is players who played in both leagues in 2024. The sample size for that is only 5, so it is not statistically significant.

```{r}
#| label: fig4-avgSwingSpeedLeague
...
-   For both leagues, the average swing speed was a little over 70 mph. This is very similar to the overall MLB average swing speed of 71.92 mph found in @battingStatsSummary.

@homeRunsLeague looks at the average number of home runs hit per batter in the American and National Leagues.

```{r}
#| label: fig5-homeRunsLeague
...
-   The American League has a slight edge but both leagues rest around 22 home runs per player. This is similar to the MLB-wide average of 21.67 in @battingStatsSummary.

-   League does not appear to be an indicator of a batter's home run production.

### Scatterplot

@homeRunsVsWalksLeaguePos displays a visualization divided into the three league designations with points plotted by walks and home runs. The points are color coded by the primary position of the MLB player.

```{r}
#| label: fig6-homeRunsVsWalksLeaguePos
...
Concerning bat speed's influence on other metrics for qualified MLB batters in the 2024 season, there are positive correlations between bat speed and home runs (@homeRunvsAvgSwingSpeed) and bat speed and strikeout % (@kvsAvgSwingSpeed). The distribution for bat speed vs. strikeout %, however, is more spread, indicating a weaker correlation. @correlationHeatmap explores more correlations concerning bat speed and additionally distinguishes the relationships between other batting metrics. Notable positive correlations include those between strikeout % and whiff % as well as between average swing speed and hard hit %.

With respect to league and position for qualified batters in the 2024 MLB season, there is no relationship between the a batter's league and his average bat speed or a batter's league and his average home runs as displayed by @avgSwingSpeedLeague and @homeRunsLeague. 

Additionally, @homeRunsVsWalksLeaguePos indicates that there is not a relationship between walks and the position one plays. There may be a relation between position and home runs hit, but the sample size might need to be expanded before making that conclusion.
